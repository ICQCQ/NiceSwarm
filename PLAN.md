...
- [ ] **M7.6 — Online lobby + NAT traversal** (design in [NETWORKING.md](NETWORKING.md)): browse/join games with **no IP:port** via a self-hosted central server on the user's VPS. Adopt **Noray** (MIT, Node/Docker — pure-GDScript `netfox.noray` client, keeps ENet + single-exe build) for hole-punch + relay fallback; build a small **Lobby Registry** companion service for the game list (the piece Noray lacks). Phase 1 = registry + relay (ships the no-IP UX), Phase 2 = direct hole-punch, Phase 3 = hardening + security pass. Keep LAN direct-IP mode. Supersedes M7.5's "port forwarding docs". *(Cloudflare Tunnel fits the registry only, not Noray's UDP — see doc §11.)*

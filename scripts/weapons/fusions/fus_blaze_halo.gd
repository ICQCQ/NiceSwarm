...
func _physics_process(delta: float) -> void:
	...
	pulse_cd -= delta
	if pulse_cd <= 0.0:
		var radius: float = (cfg.pulse_radius_base + cfg.pulse_radius_per_count * (count_level() - 1)) * fuse_area()
		var pdmg: float = cfg.pulse_dmg * fuse_damage() * (1.0 + cfg.growth * (level - 1))
		var any := false
		...
		if any:
			...
			pulse_cd = cfg.pulse_cd * fuse_rate()
		else:
			pulse_cd = 0.3
func _draw() -> void:
	if player == null or player.downed:

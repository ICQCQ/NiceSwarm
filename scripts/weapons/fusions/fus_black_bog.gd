...
func _physics_process(delta: float) -> void:
	...
	if target == null:
		cooldown = 0.2
		return
	var r := (212.0 + 8.0 * (count_level() - 1)) * fuse_area()
	...
	w.radius = r
	w.damage = 1.5 * dmg  # +20%: single-well fusion, 75%-of-combined floor
	w.pull = 160.0
	w.life = life
	...
	pud.radius = r * 0.9
	pud.damage = 1.6 * dmg  # +20%: single-well fusion, 75%-of-combined floor
	pud.max_life = life
	pud.life = life

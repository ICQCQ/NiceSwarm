...
func _physics_process(delta: float) -> void:
	...
	for i in count:
		...
		g.player = player
		g.velocity = base.rotated(deg_to_rad(cfg.spread_deg) * (i - (count - 1) / 2.0)) * (cfg.speed * (1.0 + cfg.speed_growth * (level - 1)))  # speed + range grow with level
		g.damage = dmg
...
func _on_hit(e: Node2D, pos: Vector2) -> void:
	...
	var length: float = (cfg.beam_length + cfg.beam_length_per_level * (level - 1)) * fuse_area()   # laser range buffed + grows with level
	var dmg: float = cfg.beam_dmg * fuse_damage() * (1.0 + cfg.growth * (level - 1))  # laser damage buffed
	for en in Main.instance.enemies_in_radius(pos, length + 64.0):
		var rel: Vector2 = en.global_position - pos
		var along := rel.dot(dir)
		if along >= 0.0 and along <= length and (dir * along).distance_to(rel) <= cfg.beam_width + en.radius:
			damage_dealt += dmg

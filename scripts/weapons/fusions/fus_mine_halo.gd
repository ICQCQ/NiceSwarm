...
func _physics_process(delta: float) -> void:
	...
	for k in expired:
		hit_cd.erase(k)
	var n: int = cfg.blade_count_base + count_level()
	...
	for e in Main.instance.enemies_in_radius(global_position, orbit_r + blade_r + 64.0):
		...
		for i in n:
			...
			if bp.distance_to(e.global_position) <= blade_r + e.radius:
				...
				break
	drop_cd -= delta
	if drop_cd <= 0.0 and get_tree().get_nodes_in_group("mines").size() < cfg.mine_cap_base + count_level():
		var bp: Vector2 = global_position + Vector2.from_angle(angle) * orbit_r * cfg.drop_r_ratio
		var m := MineNode.new()
		m.source_pid = player.peer_id
		m.source_weapon = self
		m.damage = cfg.mine_dmg * fuse_damage() * (1.0 + cfg.growth * (level - 1))
		m.blast_radius = cfg.mine_blast_radius * fuse_area()
		m.trigger_radius = cfg.mine_trigger_radius * fuse_area()
		m.life = cfg.mine_life * fuse_duration()
		m.position = bp
		player.get_parent().add_child(m)
		Sfx.play("mine", bp)
		drop_cd = cfg.drop_cd * fuse_rate()
func _draw() -> void:
	if player == null or player.downed:
		return
	var n: int = cfg.blade_count_base + count_level()

...
- **CI release** (`.github/workflows/build-windows.yml`): on push to `publish` (rolling `latest` prerelease) and `v*` tags, exports BOTH a release exe (`NiceSwarm.exe`, `--export-release`) and a **debug exe** (`NiceSwarm-debug.exe`, `--export-debug`) — the debug build ships verbose error reporting + stack traces and turns on the in-game F1 debug panel (`OS.is_debug_build()`). Each exe gets its own `<name>.sha256` sidecar; `update_check.gd` picks the matching one by `OS.is_debug_build()` so a debug build doesn't false-positive against the release hash. `build.ps1` (local) builds only the release exe.

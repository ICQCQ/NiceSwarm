...
- **Version — two layers.** (1) The **semantic version** (`0.9.0`) lives in three places that must stay in sync when bumped: `VERSION` const in `main.gd`, `config/version` in `project.godot`, and `application/file_version`+`product_version` in `export_presets.cfg` (the Windows exe metadata). It is *no longer shown in-game* (kept for exe metadata / project config). (2) The **displayed build identity** is `BuildVersion` (`scripts/config/build_version.gd`): `BRANCH`/`COMMIT`/`COUNT` consts default to `"dev"` and are stamped at build time by `build.ps1` + the CI workflow (`rev-parse --short HEAD` / `GITHUB_REF_NAME` / `rev-list --count HEAD`). `BuildVersion.label()` → `"Publish v. 220"` (branch + commit count); `full_label()` adds the short sha (+`*` for a dirty local build) → `"Publish v. 220 (9da5762)"`, which is what the menu footer (`game_ui.gd`) shows. CI needs `fetch-depth: 0` on every checkout or `rev-list --count` returns 1. The same label is published as a plain-text `VERSION.txt` asset next to the binaries so the launcher can display which build it will install; the launcher stamps its **own** branch+count via Go `-ldflags -X main.verBranch/verCount` (mirror title-casing: uppercase first char only — NOT `String.capitalize()`).
...
- `scripts/sfx.gd` (`Sfx` autoload) — procedural audio: synthesizes all sounds at startup (no asset files), positional 2D + flat pools, per-name throttle. Call `Sfx.play(name, pos_or_null, vol_db)`. Positional sounds use **screen-relative distance attenuation** (`_distance_db`): built-in 2D rolloff is disabled (`attenuation=0`, panning kept); full volume out to the nearest visible screen edge (+`FULL_VOL_MARGIN`), then fading over `OFF_SCREEN_FADE` px to `OFF_SCREEN_FLOOR_DB` for off-screen events. Listener = local player; 0 dB with no listener (headless).
